...
// ============================================================
// COMMANDS ROUTER (manage-article-status, publish-ready-article)
// Extracted to api/routes/commands.mjs — see docs/TO_FIX.md #80.
// create-article is handled separately above (own router, own file).
// ============================================================

const commandsRouter = createCommandsRouter({
  startCommandJob: (res, args, opts) => startCommandJob(res, args, { ...opts, cwd: KILOMBO_DIR }),
  VALID_STATUSES,
  isValidStatus,
  sanitizeInput,
});
