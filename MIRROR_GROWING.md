...
🟡 Same image also breaks your own accessibility rule

alt='' — empty. MIRROR_GROWING.md §4.6 requires "toda imagen lleva alt descriptivo." The scrape flow's step 3 (cleanup) never mentions generating or checking alt text, even though sanitizeHtml() explicitly allows img[alt] to survive. Worth adding an explicit checklist line: if the scraped body carries an <img>, either the source alt is meaningful or it needs a manually-written one — don't let empty alt="" pass silently.

🟡 No dedup step in the documented flow

MIRROR_GROWING.md §1's admission rule #3 says explicitly: "El material no está ya incorporado — consultar articles.json... para evitar duplicados." Section 8's "flujo de importación recomendado" (fetch → extract → clean → validate) skips straight from cleaning to npm test — there's no step that checks the new id/sourceUrl against existing entries first. Cheap to add, and it's literally already a rule elsewhere in your docs.

🟡 Documentation placement — this belongs partly in MIRROR_GROWING.md

Section 7 (what the password is/isn't for) fits TROUBLESHOOTING.md's stated charter (server access diagnosis). But Section 8 (selectors, extraction code, cleaning regexes) is a content-import how-to — and MIRROR_GROWING.md §2 ("Proceso de incorporación de un artículo nuevo") is the doc that's supposed to govern exactly that workflow. Right now MIRROR_GROWING.md has zero reference to these selectors — I checked, there's no cross-link either direction. A future session following MIRROR_GROWING.md §2's steps (which is the doc you said you care about more) won't know these selectors exist unless they happen to also open TROUBLESHOOTING.md. At minimum, add a one-line pointer in MIRROR_GROWING.md §2: "Para selectores de scraping por sitio, ver TROUBLESHOOTING.md §8."

Minor, non-blocking notes
The PI truncation heuristic (<section id= or <footer) is reasonable but unverified against the "meme-rub" sidebar's actual position in the DOM for every PI template variant — worth a spot-check on a couple more PI articles before trusting it blindly for Week 3.
H1/H2 aren't in sanitizeHtml()'s allowlist — confirmed the actual imported articles avoid them (they use <br>/<p> for structure), so whoever did the import already worked around this correctly, but it's worth stating explicitly in Section 8 as a constraint ("no headings survive — flatten subheadings into bold <p> text") so the next import doesn't waste time debugging why an <h2> silently vanished.