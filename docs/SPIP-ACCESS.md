...
**Historical Note:** Initial test (v0.40.1) had a typo in the username ('klimbo' instead of 'kilombo'), causing false "editor-only" conclusion. This has been corrected.
...
Use `sandbox/delete-article.mjs` to inspect or change article status:
...
**Restore from trash:**
```bash
node sandbox/delete-article.mjs --change --id 87 --status publie
```
...
- **Root cause:** Initial test had username typo ('klimbo' instead of 'kilombo')
...
- Status management script: `sandbox/delete-article.mjs`
