...
The local Management Dashboard (Express server) exposes protected endpoints that can create articles, change article status and read the audit log. For safety this deployment requires a shared secret:

- The server **requires** `KILO_SHARED_SECRET` to be set in `.env` or the environment. If it is not set the server refuses to start (fail loud).
- All protected API requests must include the HTTP header `x-kilo-secret: <secret>` where `<secret>` is the value of `KILO_SHARED_SECRET`.
- Failed or malformed auth attempts are appended to `live-write-audit.log.jsonl` with timestamp, path, method, IP and user-agent.

Practical notes:
- Do NOT commit real secrets into the repository. Use [KILOMBO/.env.example](../.env.example) as a template and fill the secret only on the runtime host.
- `/api/audit-log` is itself a protected endpoint; to view audit entries query it with the `x-kilo-secret` header.
- If you see the server refusing to start, check that `KILO_SHARED_SECRET` exists and is correctly quoted in `.env` (the `.env` parser accepts quoted values and values containing `=`).
...
node scripts/manage-article-status.mjs --change --id <N> --status poubelle
...
```bash
node scripts/manage-article-status.mjs --inspect --id <N>
...
The `scripts/manage-article-status.mjs` script was tested with Article #87 ("FINAL TEST: Creación de Artículo End-to-End Verificada — 2026-08-21"):
...
- ✅ `scripts/manage-article-status.mjs` fully operational for moving articles to trash
...
- Hardcoded paths creating workspace pollution
...
|------|---|---|
| `fs_write` | `/root/JOB-sda2/KILOMBO-SITE/KILOMBO-BUILD/...` or `/root/JOB-sda2/KILOMBO-BUILD/...` | `$LOCAL_KILOMBO_DIR/...` |
...
```
Agent attempts: fs_write /root/JOB-sda2/KILOMBO-SITE/KILOMBO-BUILD/KILOMBO/file.md "content"
Result: ✅ Tool executes (no enforcement yet)
Problem: KILOMBO typo creates unauthorized directory
```

**Scenario 2 — After hook (next session onwards):**
```
Agent attempts: fs_write /root/JOB-sda2/KILOMBO-SITE/KILOMBO-BUILD/KILOMBO/file.md "content"
Hook intercepts: Detects /KILOMBO-BUILD/ pattern
...
- **Previous sessions:** This hook was created to prevent the repeated KILOMBO/KILOMBO typo mistakes documented in earlier session summaries
...
## 11. Theme Customization: Turning a Vulnerability into a Feature

### The Discovery

During security analysis, a vulnerability was identified: **direct article publishing to SPIP was easier than submitting to the editorial review queue.** This violated the security principle that safe operations should require less effort than risky ones.

The vulnerability: anyone could automate article creation AND immediate publication with fewer steps than submitting to draft for review.

### The Advantage

However, the same capability that created the vulnerability revealed an unexpected advantage: **because we CAN automate interactions with SPIP's admin interface, we CAN ALSO automate theme customization.**

Traditional approach to changing SPIP theme presentation:
1. SSH into server
2. Edit theme files
3. Restart SPIP
4. Check results

New approach with automated scripts:
1. Run field discovery script
2. Preview changes with `--dry-run`
3. Apply changes with one command
...
**What's Customizable:**
- Homepage tab labels ("Los últimos artículos" → "Breaking News")
- Section headers (any section title)
- Sidebar widget titles
- Footer text
- Block headers
- Menu labels
- Any text element in the Escal theme

**All changes:**
- Instant (live within seconds)
- Safe (preview with `--dry-run` first)
- Reversible (change back anytime)
- No code knowledge required

### How It Works

The tools use Playwright (headless browser automation) to:

1. **probe-escal-fields.mjs** — Discover all customizable fields
   ```bash
   node scripts/probe-escal-fields.mjs --verbose
   ```
   
   Output shows:
   - Field name (what to change)
   - Field type
   - Current value
   - Location in admin menu
   - Can export to JSON

2. **customize-escal-theme.mjs** — Apply changes
   ```bash
   # Preview (no changes)
...
**Goal:** Change homepage tab from "Los últimos artículos" to "Breaking News"

```bash
# Step 1: Discover field names
node scripts/probe-escal-fields.mjs --verbose
# Output shows: titreongletderniers = tab title field

# Step 2: Preview the change (100% safe)
node scripts/customize-escal-theme.mjs --field titreongletderniers --value "Breaking News" --dry-run

# Step 3: Review screenshot
# Opens escal_update_dryrun.png
# Look good? Continue to step 4

# Step 4: Apply the change (live immediately)
node scripts/customize-escal-theme.mjs --field titreongletderniers --value "Breaking News"

# Step 5: Verify at www.kilombo.top
# Homepage now shows "Breaking News" instead of "Los últimos artículos"
```

### Why This Is Secure

✅ **Safe by default:** `--dry-run` prevents accidents
✅ **Preview first:** See screenshot before applying
✅ **No SSH needed:** Works from anywhere with credentials
✅ **Fully reversible:** Change back anytime
✅ **No direct file access:** Goes through SPIP admin interface

### Security Principle Applied

> **A vulnerability can become a feature if properly understood and controlled.**

The vulnerability (easy direct publishing) revealed that SPIP's admin interface can be reliably automated. By adding proper controls (KILO_APPROVE_PUBLISHING requirement), the same capability enables safe theme customization.

**Before:** System made dangerous operations (direct publish) easier than safe ones (review queue)
**After:** System makes safe operations (theme changes via preview) the obvious choice, with risky operations (publish bypass) requiring explicit override

### Documentation

Complete guide available at: **docs/THEME-CUSTOMIZATION.md**

Includes:
- Two-tool workflow (discover + customize)
...
The same automated access that created a publishing security vulnerability also enables user-friendly theme customization. With proper controls (dry-run, explicit overrides), automation of SPIP administration becomes a **feature, not a bug.**
