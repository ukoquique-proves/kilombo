...
|------|---------|-----------------|
| **Migrate article to SPIP (draft)** | `node scripts/migrate-to-spip.mjs --article-id slug` | `docs/MIGRATION-WORKFLOW.md` |
| **Migrate & publish immediately** | `KILO_APPROVE_PUBLISHING=true node scripts/migrate-to-spip.mjs --article-id slug --publish` | `docs/MIGRATION-WORKFLOW.md` |
| **List draft articles awaiting approval** | `node scripts/list-draft-articles.mjs --all` | `docs/VIEWING-DRAFT-ARTICLES.md` |
| **Check article status** | `node scripts/manage-article-status.mjs --inspect --id <N>` | `docs/SPIP-ARTICLE-MANAGEMENT.md` |
| **Publish draft article** | `node scripts/manage-article-status.mjs --change --id <N> --status publie` | `docs/SPIP-ARTICLE-MANAGEMENT.md` |
| **Move article to trash** | `node scripts/manage-article-status.mjs --change --id <N> --status poubelle` | `docs/SPIP-ARTICLE-MANAGEMENT.md` |
| **Customize live SPIP theme** | `node scripts/customize-escal-theme.mjs` | `docs/THEME-CUSTOMIZATION.md` |

### Article Management Scripts

The project includes comprehensive article management capabilities:

- **`scripts/migrate-to-spip.mjs`** — Migrate articles from JSON to SPIP with two modes:
  - Mode A (default): Create as draft → awaiting editorial review
  - Mode B (--publish): Create and publish immediately (live)

- **`scripts/list-draft-articles.mjs`** — Query all draft articles by section/status:
  - Find articles awaiting approval
  - Filter by section or status code
  - Get article IDs and metadata for next steps

- **`scripts/manage-article-status.mjs`** — Manage article lifecycle:
  - Inspect current status and available transitions
  - Change status (publish, reject, trash, restore)
  - Preview changes with --dry-run before committing

**See [`docs/VIEWING-DRAFT-ARTICLES.md`](docs/VIEWING-DRAFT-ARTICLES.md) for complete article management documentation.**
